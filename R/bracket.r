...
#' @param x One or more numeric or character values.
#' @param by A vector of numeric or character values. These should be sorted (from high to low or low to high... if not, an error will result).
...
#' @param warn Logical. If \code{TRUE}, then warn if \code{x} is outside the range of \code{by}.
...
bracket <- function(x, by, index=FALSE, inner=TRUE, warn=FALSE) {
